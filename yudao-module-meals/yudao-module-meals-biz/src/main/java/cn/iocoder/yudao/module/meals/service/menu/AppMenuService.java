package cn.iocoder.yudao.module.meals.service.menu;

import cn.iocoder.yudao.framework.common.pojo.PageResult;
import cn.iocoder.yudao.module.meals.controller.app.menu.vo.AppMenuPageReqVO;
import cn.iocoder.yudao.module.meals.controller.app.menu.vo.AppMenuSaveReqVO;
import cn.iocoder.yudao.module.meals.dal.dataobject.menu.MenuDO;
import jakarta.validation.Valid;

/**
 * 菜单 Service 接口
 *
 * @author 珍珍
 */
public interface AppMenuService {

    /**
     * 创建菜单
     *
     * @param createReqVO 创建信息
     * @return 编号
     */
    Long createMenu(@Valid AppMenuSaveReqVO createReqVO);

    /**
     * 更新菜单
     *
     * @param updateReqVO 更新信息
     */
    void updateMenu(@Valid AppMenuSaveReqVO updateReqVO);

    /**
     * 删除菜单
     *
     * @param id 编号
     */
    void deleteMenu(Long id);

    /**
     * 获得菜单
     *
     * @param id 编号
     * @return 菜单
     */
    MenuDO getMenu(Long id);

    /**
     * 获得菜单分页
     *
     * @param userId 用户ID
     * @param pageReqVO 分页查询
     * @return 菜单分页
     */
    PageResult<MenuDO> getMenuPage(Long userId, AppMenuPageReqVO pageReqVO);

}
